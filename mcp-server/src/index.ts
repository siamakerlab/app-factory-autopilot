...
export function buildServer(ctx: Ctx): McpServer {
  ...
  const schemas = {
    ...
    plan_submit_answers: z.object({ area: z.string(), answers: looseRecordSchema }),
    plan_apply_mock_answers: z.object({ answers: looseRecordSchema.optional(), file: z.string().optional() }),
    factory_test_prepare: z.object({ scenarios: z.array(looseRecordSchema).optional(), devices: z.array(looseRecordSchema).optional() }),
    factory_test_record_result: z.object({ scenario_id: z.string(), device_id: z.string().optional(), checks: z.array(looseRecordSchema) }),
    ...
    dependency_request: z.object({ coordinates: z.string(), requested_by: z.string().optional(), reason: z.string().optional() }),
    dependency_review_version: z.object({
      coordinates: z.string(),
      approved_version: z.string(),
      source_urls: stringArraySchema.optional(),
      notes: z.string().optional(),
    }),
    dependency_review_license: z.object({
      coordinates: z.string(),
      licenses: stringArraySchema,
      source_urls: stringArraySchema.optional(),
      notes: z.string().optional(),
    }),
    dependency_approve: z.object({ coordinates: z.string(), approved_by: z.string().optional() }),
    dependency_reject: z.object({ coordinates: z.string(), reason: z.string() }),
    approval_request: z.object({
      subject: z.string(),
      title: z.string(),
      options: z.array(looseRecordSchema),
      rationale: z.string(),
      risk: z.string().optional(),
      recommendation: z.string().optional(),
    }),
    approval_get_status: z.object({ approval_id: z.string() }),
    approval_decide: z.object({ approval_id: z.string(), decision: z.string(), decided_by: z.string().optional() }),
    ...
    placeholder_resolve: z.object({ name: z.string(), value: z.unknown().optional(), evidence_ids: stringArraySchema.optional() }),
    capability_scan: z.object({ provider: z.string().optional(), capabilities: z.array(looseRecordSchema).optional() }),
    capability_record_environment: z.object({ provider: z.string().optional(), checks: z.array(looseRecordSchema).optional() }),
    capability_install_plan: z.object({ capability_ids: stringArraySchema.optional(), provider: z.string().optional() }),
    capability_mark_installed: z.object({ capability_id: z.string(), evidence: looseRecordSchema.optional() }),
    capability_mark_declined: z.object({ capability_id: z.string(), reason: z.string().optional() }),
    ...
    task_report_failure: z.object({ task_id: z.string(), error: z.string(), normalized_error: z.string().optional() }),
  };

...
export function buildServer(ctx: Ctx): McpServer {
  ...
  const schemas = {
    ...
    factory_record_delegation: z.object({
      run_id: z.string(),
      cycle_seq: z.number().int().positive(),
      ...
      roadmap_phase: z.string().optional(),
      required_evidence: stringArraySchema.optional(),
      file_ownership: stringArraySchema.optional(),
      dangerous_tags: stringArraySchema.optional(),
      tool_availability: z.record(z.string(), z.boolean()).optional(),
      previous_failures: stringArraySchema.optional(),
    }),
    factory_record_watchdog: z.object({
      run_id: z.string(),
      cycle_seq: z.number().int().positive(),
      agent: z.string(),
  ...
  const tools: [string, string, z.ZodTypeAny, Handler][] = [
    ...
    ["factory_record_delegation", "AFA-061 agent/skill 위임 판단 기록 (직렬 실행 강제)", schemas.factory_record_delegation, factory.factoryRecordDelegation as Handler],
    ["factory_record_watchdog", "AFA-061 5분 subagent watchdog 점검 결과 기록", schemas.factory_record_watchdog, factory.factoryRecordWatchdog as Handler],

...
| AFA-036 | M4 Agent/Skill | factory review 점수화 리포트 | P0 | ⬜ |
...
- **근거**: MVP-1.md 3.15 / **의존성**: AFA-011, AFA-012 / **위험도**: 하
- **구현 범위**: 상태 저장소에서 4요소 보고(진행 상황·앞으로의 목표·다음 턴
  예정·전체 진행도 %)를 생성하는 모듈 + 진행도 가중 평균 계산기
- **완료 조건**:
  - [ ] 매 작업 사이클 종료 시 4요소 보고가 출력된다
  - [ ] 진행도가 3.15 공식(상태 가중치 0/25/50/75/100)대로 계산된다
  - [ ] `factory status`가 동일 형식을 출력한다
  - [ ] 보고가 대화 기억이 아니라 상태 저장소만으로 생성된다 (재시작 후에도
    동일 보고 재현)
...
- **지침**: 무중단의 기반은 "상태 저장소만으로 재진입 가능"(AFA-020)이다 —
  ...
  코어는 "계속할지 판정하는 함수" 하나만 노출한다. 무한 루프 방지는
...
### AFA-036 factory review 점수화 리포트 — ⬜

- **근거**: MVP-1.md 3.16 / **의존성**: AFA-013, AFA-030 / **위험도**: 중
- **구현 범위**: `core/policies/review-scoring.yaml`(영역별 검사 항목·배점
  정의) + 점수 계산기 + 리포트 생성(현재 점수 → 목표 점수 → 개선 계획 →
  수정 후 전/후 비교) + 리포트 저장(`.app-factory/reports/review-<RunID>.md`)
- **완료 조건**:
...
6. **전체 진행도**는 MVP-1.md 3.15 공식을 이 로드맵에도 적용해 계산한다:
   구현 항목(AFA-*)의 상태 가중치(⬜0 / 🟨25 / 🟧50 / 🟦75 / ✅100) 평균.
   결정 항목(D-*)은 해소 여부만 별도 표기하고 분모에서 제외한다. 매 턴 종료
   보고에 이 수치를 사용한다.

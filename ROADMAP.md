...
수단으로 사용하며, 둘 다 실패하면 공식 문서를 직접 확인한다. 플러그인·Skill·
Agent·MCP 제작/수정 작업에서는 설치된 Provider 내장 스킬(`plugin-creator`,
`skill-creator`, `skill-installer`, `openai-docs`, `review-agent` 등)을
적극 사용하도록 관리문서에 명시하고, 없으면 설치 또는 대체 절차를 안내한다.
...
- **하위 작업**:
  ...
  - AFA-061i Subagent Watchdog — 서브에이전트 응답이 지연될 때 메인이 5분에
    1회 상태를 확인하고, 실행 중/정지/응답 없음/한도 초과를 판정해 대기,
    재실행, 강제종료 후 재실행, blocker 전환 중 필요한 조치를 수행하는 정책
- **완료 조건**:
...
- **지침**: 이 항목은 `factory auto`의 기본 UX를 결정한다. "새 턴 자동 생성"을
  ...
  경우 verifier/reviewer에게 재위임한 뒤 상태를 전이한다. 서브에이전트 감시는
...
- **완료 조건**:
  ...
  - [x] 에뮬레이터 관련 부족분은 자동 준비 제안 문구("바로 준비해드릴까요?")를
    포함해 사용자에게 묻는다
  - [x] 설치 완료 스킬의 `guidance_doc` 지침이 스코프에 따라 전역 관리문서

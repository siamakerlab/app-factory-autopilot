...
1. Run all gates with `gate_run_all`, including `production_readiness`. The
   caller chooses whether the context is release mode; in release mode, the
   Placeholder gate blocks.
...
4. `factory auto` must not stop at this gate if production-readiness evidence is
   missing. When the configured production-quality areas require competitor or
   community research, roadmap reflection, clean roadmap audit,
   UX/accessibility review, in-app review/update checks, or other rubric items,
   enqueue the missing review/fix work and rerun the gate after evidence is
   recorded.
